...
export class GameScene extends Phaser.Scene {
  ...
  private houses:              ConnectionHouseFactory = new ConnectionHouseFactory(this, () => { this.isDialogOpen = true })
  ...
  create(): void {
    ...
    if (chestObj)  this.chest.spawn(chestObj.x, chestObj.y, () => { this.isDialogOpen = true })
    ...
    this.doorPrompt = this.add.text(0, 0, '[E] Enter', {
      fontSize:        '6px',
      color:           '#ffffff',
    ...
    this.agentPrompt = this.add.text(0, 0, '[Space] Chat', {
      fontSize:        '6px',
      color:           '#ffffff',
      backgroundColor: '#000000cc',
      padding:         { x: 3, y: 2 },
  ...
  private spawnAgentAt(template: AgentTemplate, x: number, y: number, name: string): Agent {
    ...
    agent.on('pointerdown', () => {
      EventBus.emit('agent-clicked', { id: agent.id, name: agent.name, templateId: agent.template.id })
    })
    this.agents.push(agent)
    this.checkAgentAttention(agent)
    agent.startWander()
    EventBus.emit('agent-spawned', { id: agent.id, name: agent.name, templateId: agent.template.id })
    return agent
  }
  ...
  private updateDoorProximity(): void {
    ...
    const nearest = this.houses.findNearestDoor(
      this.player.x, this.player.y,
      GameScene.DOOR_RADIUS,
    )
  ...
  private updateAgentProximity(): void {
    ...
    if (nearest && Phaser.Input.Keyboard.JustDown(this.spaceKey)) {
      EventBus.emit('agent-clicked', { id: nearest.id, name: nearest.name, templateId: nearest.template.id, tab: 'chat' })
    }
  }
  ...
  private enterHouse(connection: Connection): void {
    ...
    if (this.scene.get(IndoorScene.KEY)?.scene.isActive() ||
  ...
  private setupEventBusListeners(): void {
    const unsubSpawn = EventBus.on('spawn-agent', ({ templateId, name, x, y, networkId, remote }) => {
    ...
    const CHAT_RESPONSES: Record<string, string[]> = {
      ecommerce: ['Checking the order data now.', 'Running the refund workflow.', 'I\'ll verify the inventory.', 'On it — pulling Shopify data.'],
      sales:     ['Queuing that in the outreach pipeline.', 'Pulling lead data now.', 'I\'ll log that to Salesforce.', 'Searching Apollo for matches.'],
      support:   ['Opening the ticket queue.', 'I\'ll look up the customer history.', 'Escalating that right away.', 'Checking Zendesk now.'],
      marketing: ['I\'ll draft that content now.', 'Scheduling the post to Buffer.', 'Pulling engagement analytics.', 'Generating copy with OpenAI.'],
    }
    ...
    const ACK_LINES = ['On it!', 'Acknowledged.', 'Got it.', 'Processing…', 'Right away!']

    const unsubCommand = EventBus.on('command-issued', ({ id: commandId }) => {
      this.agents.forEach((agent, index) => {
        const message = ACK_LINES[index % ACK_LINES.length]
        // Stagger each agent's response slightly so they don't all pop at once.
        this.time.delayedCall(index * 180, () => {
          this.showFloatingText(agent.x, agent.y, message)
    ...
    const unsubAgentMoved = EventBus.on('agent-remote-moved', ({ localId, x, y, facing, moving }) => {
    ...
    const unsubConnLoaded = EventBus.on('land-ready', ({ placements, landObjects, connections }) => {
      const connPlacements = this.resolveConnectionPlacements(placements, connections)
      const homeObj  = landObjects.find(o => o.objectType === 'home')
      const homeConn = connections.find(c => c.id === 'home')
    ...
    this.events.once('shutdown', () => { unsubSpawn(); unsubSync(); unsubRemove(); unsubSelect(); unsubRelease(); unsubCommand(); unsubMessage(); unsubAddConn(); unsubDialogEnd(); unsubAgentMoved(); unsubRemoveConn(); unsubMoveConn(); unsubConnLoaded(); this.multiplayer.destroy() })
  ...
  private createMap(): Phaser.Tilemaps.TilemapLayer {
    ...
    // --- Grass backdrop ---
    const grassMap     = this.make.tilemap({ data: groundData, tileWidth: tileSize, tileHeight: tileSize })
    const grassTileset = grassMap.addTilesetImage('grass-tiles', 'grass-tiles', tileSize, tileSize, 0, 0)!
    grassMap.createLayer(0, grassTileset, 0, 0)!

    // --- Water layer (optional, uses water.png) ---
    if (waterData) {
      const waterMap     = this.make.tilemap({ data: waterData, tileWidth: tileSize, tileHeight: tileSize })
      const waterTileset = waterMap.addTilesetImage('water-tiles', 'water-tiles', tileSize, tileSize, 0, 0)!
