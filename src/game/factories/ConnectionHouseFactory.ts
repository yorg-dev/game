...
export class ConnectionHouseFactory {
  ...
  get connectedAppIds(): Set<string> {
    return this._connectedAppIds
  }
  ...
  preload(): void {
    ...
    this.scene.load.spritesheet('grey-brick-house', greyBrickUrl, {
      frameWidth: 96,
      frameHeight: 80,
    })
  }
  ...
  create(mapDef: MapDefinition, initialHouses: PositionedConnection[]): void {
    ...
    this.scene.input.on(
      'pointerdown',
      (_p: Phaser.Input.Pointer, hits: Phaser.GameObjects.GameObject[]) => {
  ...
  private spawnHouse(connection: Connection, x: number, y: number): void {
    ...
    sprite.on('dragend', () => {
      ...
      const overlaps = this.houses.some((h) => {
        ...
        return (
          Phaser.Math.Distance.Between(sprite.x, sprite.y, h.sprite.x, h.sprite.y) < OVERLAP_DIST
        )
      })
    ...
    sprite.on('pointerover', () => {
      if (!this._isDragging) sprite.setTint(0xddddff)
    })
    sprite.on('pointerout', () => {
      if (!this._isDragging) sprite.clearTint()
    })
