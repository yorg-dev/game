...
const PANELS: { id: PanelId; label: string; description: string }[] = [
  ...
  { id: 'new-connection',     label: 'New Connection Modal',  description: '2-step modal to add an integration' },
  { id: 'agent-popover',      label: 'Agent Popover',         description: 'Overview + chat tabs for an agent' },
  { id: 'connection-popover', label: 'Connection Popover',    description: 'Info panel for a connection house' },
  { id: 'dialog-box',         label: 'Dialog Box',            description: 'RPG-style typewriter dialog' },
  { id: 'notifications',      label: 'Notifications Modal',   description: 'Bulletin board notification list' },
...
const MOCK_DIALOG_LINES = [
  { speaker: 'Merchant', speakerColor: '#96bf48', text: 'Welcome to the Shopify house. Your store data is synced and ready.' },
  { speaker: 'Merchant', speakerColor: '#96bf48', text: 'You can place your agents here to handle orders, refunds, and fulfilment tasks automatically.' },
  { speaker: 'System',   speakerColor: '#c8974c', text: 'All actions are logged. Press [Space] to continue.' },
]
...
function NewAgentPanel() {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {open && (
        <NewAgentModal
          onSubmit={(name, templateId) => { setLast(`${name} (${templateId})`); setOpen(false) }}
...
function NewConnectionPanel() {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      {open && (
        <NewConnectionModal
          onSubmit={(appId, label) => { setLast(`${label} (${appId})`); setOpen(false) }}
          onCancel={() => setOpen(false)}
        />
      )}
    </div>
...
function DialogBoxPanel() {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <p className="text-xs text-gray-500">Renders at bottom of screen — click or press Space to advance</p>
      {open && (
        <DialogBox lines={MOCK_DIALOG_LINES} onClose={() => setOpen(false)} />
      )}
    </div>
  )
}
...
function NotificationsPanel() {
  return (
    <div className="flex flex-col items-center gap-4">
      <DebugButton onClick={() => EventBus.emit('sign-clicked', undefined)}>Open Notifications Modal</DebugButton>
      <NotificationsModal />
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex flex-col items-center gap-1">
        ...
        <p className={`text-sm font-mono font-bold ${seen ? 'text-emerald-400' : 'text-amber-400'}`}>
      ...
      <p className="text-xs text-gray-600 max-w-xs text-center">
        After resetting, reload the game page — the 3-line welcome dialog will play automatically on first scene creation.
      </p>
    </div>
  )
}
...
function PanelRenderer({ id }: { id: PanelId }) {
  switch (id) {
    case 'new-agent':          return <NewAgentPanel />
    case 'new-connection':     return <NewConnectionPanel />
    case 'agent-popover':      return <AgentPopoverPanel />
    case 'connection-popover': return <ConnectionPopoverPanel />
    case 'dialog-box':         return <DialogBoxPanel />
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col font-sans">
      ...
      <header className="flex items-center gap-3 px-5 py-3 border-b border-gray-800 bg-gray-900 shrink-0">
        <span className="text-base font-bold font-mono text-emerald-400 tracking-tight">⬡ Component Lab</span>
        <span className="text-gray-600 text-xs">— dev only · <span className="font-mono">/#/debug</span></span>
      </header>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <nav className="w-56 shrink-0 border-r border-gray-800 bg-gray-900 flex flex-col py-3 gap-0.5">
          <p className="px-4 text-[10px] uppercase tracking-widest text-gray-600 pb-1">Components</p>
          {PANELS.map(p => (
            <button
              key={p.id}
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="shrink-0 border-t border-gray-800 bg-gray-900">
            <div className="flex items-center gap-2 px-4 pt-3 pb-1">
              <span className="text-[10px] uppercase tracking-widest text-gray-500 font-mono">Live CSS Override</span>
              {customCss.trim() && (
