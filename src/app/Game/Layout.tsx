...
export const GameLayout = ({ children }: CoreLayoutProps) => {
  ...
  useEffect(() => {
    async function resolveStartingLand(urlLandId: string | null) {
      if (urlLandId) {
        try {
          const { data: land } = await dataProvider.getOne<Land>('lands', { id: urlLandId })
          if (land?.id === urlLandId) return land
        } catch {
          // fall through to my_land
        }
      }
      try {
        const { data: land } = await dataProvider.getOne<Land>('my_land', { id: '' })
        return land
      } catch {
        return null
      }
    ...
    async function loadLand() {
      ...
      console.debug('[App:loadLand] land:', land.id, '| objects:', landObjects)
