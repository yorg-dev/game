...
export function LoginModal({ onSuccess, onCancel, defaultTab = 'login' }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err?.message ?? (tab === 'login' ? 'Login failed. Please check your credentials.' : 'Registration failed. Please try again.'))
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="px-5 pt-5 pb-3 border-b-4 border-[#7a5230] bg-[#dcc898] pr-14">
          <h2 className="text-[#3d2010] font-bold text-base">{isLogin ? 'Load World' : 'Save Progress'}</h2>
          <p className="text-xs text-[#7a5230] pt-0.5">
            {isLogin ? 'Sign in to continue your saved world.' : 'Create an account to save your progress.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-5 py-5">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-[#7a5230] uppercase tracking-widest">Email</label>
            <input
              autoFocus
              type="email"
              value={email}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-[#7a5230] uppercase tracking-widest">Password</label>
            <input
              type="password"
              value={password}
